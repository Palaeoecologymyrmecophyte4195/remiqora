...
- **Общее хранилище**: все треки (сгенерированные, загруженные, собранные в редакторе) лежат в одной SQLite-базе и доступны из любого модуля — Demucs, MIDI, микшер и редактор работают с одной и той же библиотекой, а не тремя разными.
...
- **Доперевести композицию** — то же самое, но сразу для списка партий.

Плюс: длительность 10–300 сек, батч 1/2/4 варианта, форматы mp3/wav/flac, расширенные параметры (BPM, тональность, размер такта, шаги инференса, guidance scale, seed), поддержка LoRA-адаптеров с регулировкой силы, локальные пресеты параметров и кнопка «Остановить всё» для массовой отмены задач.
...
![Микшер стемов](docs/screenshots/09-mixer.png)

Фиксированная 4-канальная консоль (вокал/барабаны/бас/остальное + мастер) для быстрой сведения стемов: громкость, панорама, mute/solo, 3-полосный EQ, компрессор, реверб, VU-метры с индикацией клиппинга. Настройки сохраняются автоматически. Кнопка **«Открыть в редакторе»** переносит все 4 стема с текущими настройками в новый проект полноценного DAW — микшер задуман как быстрый предпросмотр, редактор — как его надмножество.

## Встроенный DAW

![Редактор с клипом на таймлайне](docs/screenshots/08-editor-with-clip.png)
...
**Драйвер NVIDIA GPU сюда сознательно не входит** — поставьте вручную с
...
5. Создаёт `backend/.env` с путями на только что склонированные репозитории.
...
`-SkipWeights` пропускают соответствующие шаги). Нужны заранее установленные:
...
После этого вручную остаётся только:
- поставить ffmpeg и указать `FFMPEG_BIN_DIR` в `backend/.env`;
- проверить `CUDA_BIN_DIR` в `backend/.env`.
...
Для сборки стемов отдельно нужен ещё один uv-проект с Demucs — поднимается так же, как ACE-Step (`uv init` + `uv add demucs` в отдельной папке), веса `htdemucs` скачиваются сами при первом использовании.

...
**Статус.** Экспериментально. Установщики пока не подписаны: Windows показывает предупреждение SmartScreen («Подробнее» → «Выполнить в любом случае»), а macOS может попросить разрешить приложение («Всё равно открыть» в «Системные настройки → Конфиденциальность и безопасность»). Сумма SHA-256 каждого файла лежит в `SHA256SUMS.txt` на странице релиза. Чтобы собрать установщик самостоятельно:
